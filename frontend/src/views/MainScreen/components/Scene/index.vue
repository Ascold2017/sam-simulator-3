<template>
    <TresCanvas window-size antialias alpha shadows preset="realistic">
        ...
        <TresDirectionalLight :color="0xffffff" :intensity="1.2"
            :position="[Math.sin(sunElevationRad) * 500, Math.cos(sunElevationRad) * 500, 0]" castShadow
            :shadow-mapSize="[2048, 2048]" :shadow-camera-left="-500" :shadow-camera-right="500"
            :shadow-camera-top="500" :shadow-camera-bottom="-500" :shadow-camera-near="0.5" :shadow-camera-far="2000"
            :shadow-radius="4" :shadow-bias="-0.0005" />
        ...
        <!-- Летающие обьекты -->
        <FlightObject v-for="flightObject in parsedFlightObjects" :flight-object="flightObject" :key="flightObject.id" />
        <SmokeTrails />
